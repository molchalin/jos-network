...
sys_alloc_region(envid_t envid, uintptr_t addr, size_t size, int perm) {
    ...
    if (CLASS_MASK(0) & addr) return -E_INVAL;
...
sys_map_region(envid_t srcenvid, uintptr_t srcva,
    ...
    if (CLASS_MASK(0) & srcva || CLASS_MASK(0) & dstva) return -E_INVAL;

    if (srcva >= MAX_USER_ADDRESS) return -E_INVAL;
    if (dstva >= MAX_USER_ADDRESS) return -E_INVAL;

    perm |= PROT_USER_;

    if (perm & ~PROT_ALL || perm & ALLOC_ZERO || perm & ALLOC_ONE) return -E_INVAL;

    //user_mem_assert(srcenv, (void*) srcva, size, perm);
    //user_mem_assert(dstenv, (void*) dstva, size, perm);
...
sys_unmap_region(envid_t envid, uintptr_t va, size_t size) {
    ...
    if (CLASS_MASK(0) & va) return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, uintptr_t srcva, size_t size, int perm) {
    ...
    if (srcva < MAX_USER_ADDRESS && to_env->env_ipc_dstva < MAX_USER_ADDRESS) {
        ...
        //res = map_region(&to_env->address_space, to_env->env_ipc_dstva, NULL, 0, PAGE_SIZE, perm|ALLOC_ZERO|PROT_USER_|PROT_LAZY);
        //if (res < 0) {
        //    return res;
        //}
