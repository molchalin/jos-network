...
acpi_find_table(const char *sign) {
    ...
    if (!rsdt) {
        ...
        assert(!checksum((uint8_t *)rsdp, 20));

        uintptr_t rsdt_ptr = (rsdp->Revision == 0) ? (uintptr_t)rsdp->RsdtAddress : (uintptr_t)rsdp->XsdtAddress;
        rsdt_entsz = (rsdp->Revision == 0) ? sizeof(uint32_t) : sizeof(uint64_t);

        rsdt = (RSDT *)mmio_map_region(rsdt_ptr, sizeof(ACPISDTHeader));
        assert(!checksum((uint8_t *)rsdt, rsdt->h.Length));
    }


    int entries = (rsdt->h.Length - sizeof(RSDT)) / rsdt_entsz;

    for (int i = 0; i < entries; i++) {
        uintptr_t addr;
        memcpy(&addr, (uint8_t *)rsdt->PointerToOtherSDT + i * rsdt_entsz, rsdt_entsz);
        ACPISDTHeader *h = (ACPISDTHeader *)mmio_map_region(addr, sizeof(ACPISDTHeader));
        h = mmio_map_region(addr, h->Length);
        assert(!checksum((uint8_t *)h, h->Length));
