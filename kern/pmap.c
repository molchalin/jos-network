...
alloc_child(struct Page *parent, bool right) {
    ...
    if (right) new->addr += (1ULL << (parent->class - 1));
    ;
...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    if (trace_memory_more) cprintf("Attaching memory region [%08lX, %08lX] with type %08x\n", start, end - 1, type);
    ...
    while (start != end) {
        ...
        for (class = 0; class < MAX_CLASS; class ++) {
            if (start & CLASS_MASK(class + 1) || CLASS_SIZE(class + 1) > end - start) {
                break;
            }
        }
...
dump_memory_lists(void) {
    for (int i = 0; i < MAX_CLASS; i++) {
        struct List *list = &free_classes[i];
        if (!list_empty(list)) {
            for (struct List *n = list->next; n != list; n = n->next) {
                struct Page *page = (struct Page *)n;
                cprintf("[%08lX %08lX] class: %d\n", (long)page->addr, page->addr + (long)CLASS_MASK(page->class), page->class);
            }
        }
    }
}
...
inline static int
map_physical_region_check(struct AddressSpace *dst, physaddr_t pstart, physaddr_t pend, uintptr_t vstart, uintptr_t vend, int flags) {
    assert(pstart - pend == vstart - vend);
    return map_physical_region(dst, vstart, pstart, pend - pstart, flags);
}
...
init_memory(void) {
    ...
    res = map_physical_region_check(&kspace,
                                    0, max_memory_map_addr,
                                    KERN_BASE_ADDR, KERN_BASE_ADDR + max_memory_map_addr,
                                    ALLOC_WEAK | PROT_W | PROT_R);
    if (res) panic("cannot map kernel onto kernel address space");
    ...
    res = map_physical_region_check(&kspace,
                                    PADDR(__text_start), PADDR(__text_end),
                                    (uintptr_t)__text_start, (uintptr_t)__text_end,
                                    PROT_R | PROT_X);
    if (res) panic("cannot map .text onto kernel address space");
    ...
    res = map_physical_region_check(&kspace,
                                    PADDR(bootstack), PADDR(bootstack) + KERN_STACK_SIZE,
                                    KERN_STACK_TOP - KERN_STACK_SIZE, KERN_STACK_TOP,
                                    PROT_R | PROT_W);
    if (res) panic("cannot map kernel stack onto kernel address space");
    // Map [PADDR(pfstack), PADDR(pfstack) + KERN_PF_STACK_SIZE] to [KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE, KERN_PF_STACK_TOP] as RW-
    res = map_physical_region_check(&kspace,
                                    PADDR(pfstack), PADDR(pfstack) + KERN_PF_STACK_SIZE,
                                    KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE, KERN_PF_STACK_TOP,
                                    PROT_R | PROT_W);
    if (res) panic("cannot map kernel pf stack onto kernel address space");
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            res = map_physical_region_check(&kspace,
                                            mstart->PhysicalStart, mstart->PhysicalStart + mstart->NumberOfPages * PAGE_SIZE,
                                            mstart->VirtualStart, mstart->VirtualStart + mstart->NumberOfPages * PAGE_SIZE,
                                            PROT_R | PROT_W);
            if (res) panic("cannot map uefi memory map");
        }
    }
    ...
    * This must be done as early as possible */
    ...
    res = map_physical_region_check(&kspace,
                                    uefi_lp->FrameBufferBase, uefi_lp->FrameBufferBase + uefi_lp->FrameBufferSize,
                                    FRAMEBUFFER, FRAMEBUFFER + uefi_lp->FrameBufferSize,
                                    PROT_R | PROT_W | PROT_WC);
    if (res) panic("cannot map framebuffer");
    ...
    res = map_physical_region_check(&kspace,
                                    0, MIN(MAX_LOW_ADDR_KERN_SIZE, max_memory_map_addr),
                                    X86ADDR(KERN_BASE_ADDR), X86ADDR(KERN_BASE_ADDR) + MIN(MAX_LOW_ADDR_KERN_SIZE, max_memory_map_addr),
    ...
    res = map_physical_region_check(&kspace,
                                    PADDR(__text_start), ROUNDUP(PADDR(__text_end), CLASS_SIZE(0)),
                                    X86ADDR((uintptr_t)__text_start), ROUNDUP(X86ADDR((uintptr_t)__text_end), CLASS_SIZE(0)),
                                    PROT_X | PROT_R);
    if (res) panic("cannot map .text");
    ...
    res = map_physical_region_check(&kspace,
                                    ...
                                    X86ADDR(KERN_STACK_TOP - KERN_STACK_SIZE), X86ADDR(KERN_STACK_TOP),
                                    PROT_R | PROT_W);
    if (res) panic("cannot map bootstack");
    // Map [X86ADDR(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE), KERN_PF_STACK_TOP] to
    //     [PADDR(pfstack), PADDR(pfstacktop)] as RW-
    res = map_physical_region_check(&kspace,
                                    PADDR(pfstack), PADDR(pfstacktop),
                                    X86ADDR(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE), X86ADDR(KERN_PF_STACK_TOP),
                                    PROT_R | PROT_W);
    if (res) panic("cannot map pfstack");
