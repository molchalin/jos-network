...
env_init(void) {
    ...
    for (struct Env *env = envs + (NENV - 1); env >= envs; env--) {
        env->env_link = env_free_list;
        env_free_list = env;
        env->env_id = 0;
        env->env_parent_id = 0;
        env->env_type = ENV_TYPE_IDLE;
        env->env_status = 0;
        env->env_runs = 0;
        env->binary = NULL;
...
env_alloc(struct Env **newenv_store, envid_t parent_id, enum EnvType type) {
    ...
    uintptr_t stack_top = 0x2000000 - 2 * PAGE_SIZE * (env - envs);
...
bind_functions(struct Env *env, uint8_t *binary, size_t size, uintptr_t image_start, uintptr_t image_end) {
    ...
    struct Secthdr *sym_tab_header = NULL;
    struct Secthdr *str_tab_header = NULL;

    struct Secthdr *sh = (struct Secthdr *)(binary + elf->e_shoff);
    uint16_t shnum = elf->e_shnum;

    for (; shnum && !(sym_tab_header && str_tab_header); shnum--) {
        if (!sym_tab_header && sh->sh_type == ELF_SHT_SYMTAB) {
            sym_tab_header = sh;
        } else if (!str_tab_header && sh->sh_type == ELF_SHT_STRTAB) {
            str_tab_header = sh;
        }
        sh++;
    }
    if (!sym_tab_header || !str_tab_header) {
        return -E_UNSPECIFIED;
    }
    struct Elf64_Sym *sym_start = (struct Elf64_Sym *)(binary + sym_tab_header->sh_offset);
    struct Elf64_Sym *sym_end = (struct Elf64_Sym *)(binary + sym_tab_header->sh_offset + sym_tab_header->sh_size);
    char *str_start = (char *)(binary + str_tab_header->sh_offset);

    for (struct Elf64_Sym *sym = sym_start; sym != sym_end; sym++) {
        if (ELF64_ST_BIND(sym->st_info) != STB_GLOBAL) continue;
        if (ELF64_ST_TYPE(sym->st_info) != STT_OBJECT) continue;

        char *fname = str_start + sym->st_name;
        uintptr_t addr = find_function(fname);
        if (!addr) continue;

        cprintf("found addr for %s: %08lx\n", fname, addr);
        uintptr_t *ref = (uintptr_t *)sym->st_value;
        *ref = addr;
    }
...
load_icode(struct Env *env, uint8_t *binary, size_t size) {
    ...
    for (uint16_t i = 0; i < elf->e_phnum; i++) {
        ...
        int res = map_region(&env->address_space, start_aligned, NULL, 0, end_aligned - start_aligned, PROT_RWX | PROT_USER_ | ALLOC_ZERO);
        if (res) return res;
        memset((void *)ph->p_va, 0, ph->p_memsz);
        void *foffset = (void *)(binary + ph->p_offset);
        memcpy((void *)ph->p_va, foffset, ph->p_filesz);
    }
    switch_address_space(&kspace);
    env->env_tf.tf_rip = elf->e_entry;

    // LAB 8: Your code here
    int res = bind_functions(env, binary, size, 0, 0);
    if (res) return res;

    return map_region(&env->address_space, USER_STACK_TOP - USER_STACK_SIZE, NULL, 0, USER_STACK_SIZE, PROT_RWX | PROT_USER_ | ALLOC_ZERO);
}
...
env_create(uint8_t *binary, size_t size, enum EnvType type) {
    ...
    int status = env_alloc(&result, 0, type);
    // LAB 8: Your code here
    // LAB 3: Your code here
    ...
    status = load_icode(result, binary, size);
    if (status < 0) {
        panic("cant load icode\n");
    }
}
