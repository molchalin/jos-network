...
debuginfo_rip(uintptr_t addr, struct Ripdebuginfo *info) {
    ...
    * string returned by function_by_info will always be */
    ...
    info->rip_fn_namelen = MIN(strlen(tmp_buf), sizeof(info->rip_file));

error:
    if (old_cr3 != kspace.cr3) lcr3(old_cr3);
    return res;
}

int
dwarf_find_function(const char *const fname, uintptr_t *offset) {
    struct Dwarf_Addrs addrs;
    load_kernel_dwarf_info(&addrs);
...
find_function(const char *const fname) {
    ...
    struct Elf64_Sym *sym_tab_start = (struct Elf64_Sym *)(uefi_lp->SymbolTableStart);
    struct Elf64_Sym *sym_tab_end = (struct Elf64_Sym *)(uefi_lp->SymbolTableEnd);
    char *str_tab_start = (char *)(uefi_lp->StringTableStart);
    for (struct Elf64_Sym *sym = sym_tab_start; sym != sym_tab_end; sym++) {
        char *name = str_tab_start + sym->st_name;
    ...
    uintptr_t offset;
    int status = dwarf_find_function(fname, &offset);
