...
debug_prot(int prot) {
    cprintf("%s %s %s %s %s %s %s %s %s %s\n",
            ...
            (prot & PROT_W) ? "PROT_W" : "-",
            (prot & PROT_R) ? "PROT_R" : "-",
            (prot & PROT_WC) ? "PROT_WC" : "-",
            (prot & PROT_CD) ? "PROT_CD" : "-",
            (prot & PROT_SHARE) ? "PROT_SHARE" : "-",
            (prot & PROT_LAZY) ? "PROT_LAZY" : "-",
            (prot & PROT_COMBINE) ? "PROT_COMBINE" : "-",
            (prot & ALLOC_ZERO) ? "ALLOC_ZERO" : "-",
            (prot & ALLOC_ONE) ? "ALLOC_ONE" : "-");
}
...
map_segment(envid_t child, uintptr_t va, size_t memsz,
    ...
    size_t filesz_a = ROUNDUP(va + filesz, PAGE_SIZE) - va;

    if (memsz > filesz_a) {
        res = sys_alloc_region(child, (void *)va + filesz_a, memsz - filesz_a, perm);
        if (res < 0) return res;
    }


    res = sys_alloc_region(CURENVID, UTEMP, filesz_a, PROT_RW);
    if (res < 0) return res;

    if (filesz_a > filesz) {
        memset(UTEMP + filesz, 0, filesz_a - filesz);
    }
    ...
    res = sys_unmap_region(CURENVID, UTEMP, filesz_a);
    if (res < 0) return res;
